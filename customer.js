function validateCustomer() {
    ...
    // Address validation: max 100 chars, not empty
    if (address.length === 0 || address.length > 100) {
        showError("Address is required and must be maximum 100 characters");
        return false;
    }
    ...
    try {
        const newCustomer = auth.registerCustomer({
            ...
            contactEmail: email, // Store the actual email for contact purposes
            password: password,
            address: address,
            contact: contact
        });
